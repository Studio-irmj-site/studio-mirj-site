(() => {
  ...
  function errorText(m) {
    ...
    if (/too many requests/i.test(s)) return 'Muitas tentativas. Aguarde alguns minutos.';
  ...
  function openServiceForm(id){
    const s=id?services.find(x=>String(x.id)===String(id)):null;
    const c=$('content');
    ...
    $('serviceForm').onsubmit=async e=>{
      ...
      try{
        const result=s?await client.from('services').update(payload).eq('id',s.id):await client.from('services').insert(payload);
        if(result.error) throw result.error;
        await renderServices();
        alert(s?'Serviço atualizado com sucesso!':'Serviço cadastrado com sucesso!');
