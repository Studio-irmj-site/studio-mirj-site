...
function selectedServices() {
  return state.services.filter((service) => state.selectedIds.has(serviceId(service)));
}
...
function studioName() {
  return document.querySelector("#brandName").textContent.trim() || "Studio I.R";
}
...
function whatsappMessage() {
  ...
  if (!chosen.length) {
    return `Olá! Vim pelo site do ${studioName()} e gostaria de saber mais sobre os serviços e horários.`;
  }
  ...
  return [
    `Olá! Vim pelo site do ${studioName()} e gostaria de agendar:`,
    "",
    serviceLines,
    "",
    `Valor estimado: ${formatMoney.format(selectedTotal())}`,
    "",
...
function updateQuoteSummary() {
  ...
  elements.count.textContent = amount
    ? `${amount} ${amount === 1 ? "serviço selecionado" : "serviços selecionados"}`
    : "Nenhum serviço selecionado";

  elements.total.textContent = amount
    ? `Estimativa: ${formatMoney.format(selectedTotal())}`
...
function filteredServices() {
  if (state.activeCategory === "Todos") return state.services;
  return state.services.filter((service) => service.category === state.activeCategory);
}
...
function createServiceCard(service) {
  ...
  card.setAttribute(
    "aria-label",
    `${selected ? "Remover" : "Adicionar"} ${service.name}, ${formatMoney.format(Number(service.price || 0))}`,
  );
  ...
  card.addEventListener("click", () => {
    ...
    card.setAttribute(
      "aria-label",
      `${state.selectedIds.has(id) ? "Remover" : "Adicionar"} ${service.name}, ${formatMoney.format(Number(service.price || 0))}`,
    );
    updateQuoteSummary();
  });
...
function renderServices() {
  ...
  elements.servicesStatus.textContent = visibleServices.length
    ? `${visibleServices.length} ${visibleServices.length === 1 ? "serviço disponível" : "serviços disponíveis"}`
    : "Nenhum serviço nesta categoria";

  if (!visibleServices.length) {
    const empty = document.createElement("div");
    empty.className = "empty-state";
    empty.innerHTML = state.services.length
      ? "<strong>Nenhum serviço por aqui</strong><p>Escolha outra categoria para continuar.</p>"
      : "<strong>Novidades em breve</strong><p>Não há serviços disponíveis no momento. Fale conosco pelo WhatsApp para saber mais.</p>";
...
function showLoadError() {
  ...
  error.innerHTML = [
    ...
    "<p>Verifique sua conexão e tente carregar os serviços novamente.</p>",
...
async function loadServices() {
  ...
  try {
    const services = await fetchTable(
      "/services?select=id,name,category,description,price,unit&active=eq.true",
    );
...
function applyStudioSettings(settings) {
  ...
  if (settings.studio_name) {
    ...
    document.querySelector("#footerName").textContent = `${settings.studio_name} — Iarytsa e Raquel`;
  }
  ...
  if (settings.hours) {
    ...
    if (timeParts.length) {
      ...
      document.querySelector("#hours").textContent = settings.hours;
    }
  }
...
async function loadStudioSettings() {
  try {
    const settings = await fetchTable(
      "/studio_settings?select=studio_name,whatsapp,instagram,city,address,hours,tagline&id=eq.1",
    );
    applyStudioSettings(settings[0]);
  } catch (error) {
    console.warn("Configurações do Studio indisponíveis; usando os dados padrão.", error);
  }
}
