...
function whatsappMessage() {
  ...
  const serviceLines = chosen.map((service) => { const unit = service.unit ? ` ${service.unit}` : ""; return `• ${service.name} — ${formatMoney.format(Number(service.price || 0))}${unit}`; }).join("\n");
...
function categories() { return ["Todos", ...new Set(state.services.map((service) => service.category).filter(Boolean))]; }
function filteredServices() { return state.activeCategory === "Todos" ? state.services : state.services.filter((service) => service.category === state.activeCategory); }
function renderCategoryFilters() {
  elements.tabs.replaceChildren();
  categories().forEach((category) => {
    const button = document.createElement("button"); button.className = "category-button"; button.type = "button"; button.textContent = category; button.setAttribute("aria-pressed", String(category === state.activeCategory));
    button.addEventListener("click", () => { state.activeCategory = category; renderCategoryFilters(); renderServices(); });
...
function createServiceCard(service) {
  ...
  if (service.description) { const description = document.createElement("p"); description.textContent = service.description; copy.append(description); }
  const price = document.createElement("span"); price.className = "service-card__price";
  const priceValue = document.createElement("strong"); priceValue.textContent = formatMoney.format(Number(service.price || 0)); price.append(priceValue);
  if (service.unit) { const unit = document.createElement("small"); unit.textContent = service.unit; price.append(unit); }
  const check = document.createElement("span"); check.className = "service-card__check"; check.setAttribute("aria-hidden", "true"); check.textContent = "✓";
...
function renderServices() {
  const visibleServices = filteredServices(); elements.services.replaceChildren(); elements.services.setAttribute("aria-busy", "false");
  elements.servicesStatus.textContent = visibleServices.length ? `${visibleServices.length} ${visibleServices.length === 1 ? "serviço disponível" : "serviços disponíveis"}` : "Nenhum serviço nesta categoria";
  if (!visibleServices.length) { const empty = document.createElement("div"); empty.className = "empty-state"; empty.innerHTML = state.services.length ? "<strong>Nenhum serviço por aqui</strong><p>Escolha outra categoria para continuar.</p>" : "<strong>Novidades em breve</strong><p>Não há serviços disponíveis no momento. Fale conosco pelo WhatsApp para saber mais.</p>"; elements.services.append(empty); return; }
  visibleServices.forEach((service) => elements.services.append(createServiceCard(service)));
}
function showLoadError() {
  elements.tabs.replaceChildren(); elements.services.setAttribute("aria-busy", "false"); elements.servicesStatus.textContent = "Não foi possível carregar os serviços";
  const error = document.createElement("div"); error.className = "error-state"; error.innerHTML = ["<strong>Algo não saiu como esperado</strong>", "<p>Verifique sua conexão e tente carregar os serviços novamente.</p>", '<button class="retry-button" type="button">Tentar novamente</button>'].join("");
  error.querySelector("button").addEventListener("click", loadServices); elements.services.replaceChildren(error);
}
...
async function loadServices() {
  ...
  try {
    ...
    state.services = services.sort((first, second) => { const categoryOrder = String(first.category || "").localeCompare(String(second.category || ""), "pt-BR"); return categoryOrder || String(first.name || "").localeCompare(String(second.name || ""), "pt-BR"); });
    const validIds = new Set(state.services.map(serviceId)); state.selectedIds.forEach((id) => { if (!validIds.has(id)) state.selectedIds.delete(id); });
    state.activeCategory = "Todos"; renderCategoryFilters(); renderServices(); updateQuoteSummary();
  } catch (error) { console.error(error); showLoadError(); }
}
function normalizePhone(phone) { const digits = String(phone || "").replace(/\D/g, ""); if (digits.length === 10 || digits.length === 11) return `55${digits}`; return digits.length >= 12 ? digits : DEFAULT_PHONE; }
...
function applyStudioSettings(settings) {
  ...
  if (settings.hours) { const [days, ...timeParts] = String(settings.hours).split("·").map((part) => part.trim()); if (timeParts.length) { document.querySelector("#hoursTitle").textContent = days; document.querySelector("#hours").textContent = timeParts.join(" · "); } else document.querySelector("#hours").textContent = settings.hours; }
